...
def run_eda():
    ...
    st.text('상담유형을 선택하면 데이터프레임과 함께, 해당 유형이 가장 많은 지역순으로 보여드립니다.')
    ...
    if selected_columns:
        ...
        for column in selected_columns:
            ...
            # 파이 차트 그리기
            fig_pie = px.pie(df_filtered, names='지역', values=column, title=f'{column} 관련 유형 상담의 지역별 퍼센테이지', hole=0.2,)
    ...
    st.text('지역을 선택하면 선택한 데이터프레임과 함께 가장 많은 상담유형순서로 보여드립니다.')
